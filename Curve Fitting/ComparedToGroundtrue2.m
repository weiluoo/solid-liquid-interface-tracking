...
for iNum = startNum:3:endNum
    image = imread([originPath imageFiles(iNum+1).name]);
    iTrace = AllMaps(:,:,iNum-s+2);
    jNum = iNum;
    gt = csvread([groundtruePath num2str(jNum) '.csv']);
    [ix, iy] = find(iTrace);
    jx = gt(:,7); jy = gt(:,6);
    ...
    dists = dists .* valids;
    disp(num2str(mean(dists)));
    disp(num2str(1 - mean(dists ./ crossPoints(:,1))));
