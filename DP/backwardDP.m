...
for iNum = index-1:-1:1
    ...
    if prevY(1) - 10 <= 1
        ...
        startArea = costMap(prevX(1)-1:prevX(1), prevY(1)-10:prevY(1)-1);
        [~, idxY] = max(sum(startArea, 1));
        startCol = prevY(1) + idxY - 11;
    end
    endArea = costMap(prevX(end)-4:prevX(end), prevY(end)-10:prevY(end)-2);
    [~, idxY] = max(sum(endArea, 1));
    endCol = prevY(end) + idxY - 11;
