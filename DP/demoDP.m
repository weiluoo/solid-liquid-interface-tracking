...
load('/usr/local/home/wlhz2/Documents/MATLAB/Boundaries Segment App/Results/80_Al_CastSubstrate_P100S0.5_U18G15_500ns_S1/demo.mat', 'map', 'index');
...
% % % get the search area % % % 
startCol = min(y); endCol = max(y);
minRow = min(x) - thred; maxRow = max(x) + thred;

% % % get the mask % % % 
eudist = bwdist(map, 'euclidean');
mask = eudist < thred;
...
AllMaps = zeros(rows, cols, n);
AllMaps(:,:,index) = iMap;

% save('/usr/local/home/wlhz2/Documents/MATLAB/Boundaries Segment App/Results/80_Al_CastSubstrate_P100S0.5_U18G15_500ns_S1/AllMaps.mat', 'AllMaps', 'index');