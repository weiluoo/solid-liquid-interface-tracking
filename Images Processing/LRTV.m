...
addpath(genpath('Denoising Hyperspectral Image'));
...
demo = imread([originPath imageFiles(50).name]);
[rows, cols] = size(demo);
N = size(imageFiles, 1);
s = 100;
e = 250;
N = e - s + 1;
Hyp = zeros(rows, cols, N);

for iNum = s:e
    image = imread([originPath imageFiles(iNum).name]);
    Hyp(:,:,iNum - s + 1) = image - demo;
end

% Read the images and take a subimage of rows*cols*N
...
[F_hat, F_h, ~] = LRTV_11(Observe, 1/640, 0.01*weight, 1e-8, 1000, row, col); % with TV_norm
...
% save('/usr/local/home/wlhz2/Documents/MATLAB/Boundaries Segment App/Data/80_Al_CastSubstrate_P100S0.5_U18G15_500ns_S1/80_contrast_rmbg_640.mat', 'F', 's', 'e');
